...
#define CUBLASASSERT(status_)                                                                      \
  {                                                                                                \
    ...
    if (__s != CUBLAS_STATUS_SUCCESS) {                                                             \
      DEBUG_PRINTF("%s %s %d: %d\n", __FILE__, __FUNCTION__, __LINE__, __s);                        \
    }                                                                                              \
  }
...
// For full mask mode, we must produce the compressed mask format expected by the fused attention
// path. Currently, only two sequence lengths are supported. We hard code the sizes here. The number
// of threads per CTA: warps_m * warps_n * warps_k * 32;
constexpr size_t threadsPerCta128 = 2 * 2 * 32;
constexpr size_t threadsPerCta384 = 1 * 8 * 32;

// The number of xmmas in the M dimension. We use one uint32_t per XMMA in the M dimension: (s +
// 16*warps_m - 1) / (16*warps_m);
...
template <typename T>
inline void
copyToDevice(WeightsWithOwnership& hostWeights, size_t nbBytes, cuda_unique_ptr<T>& cudaWeights) {
