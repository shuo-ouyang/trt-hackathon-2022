...
tmap['5135'].values = np.array([64])
tmap['5136'].values = np.array([128])
tmap['5137'].values = np.array([320])
tmap['5360'].values = np.array([512])
...
for node in graph.nodes:
    if node.name in ('Reshape_331', 'Reshape_545', 'Reshape_759', 'Reshape_1080', 'Reshape_1294','Reshape_1508'):
        idx = node.name.split('_')[-1]
        slice_output0 = gs.Variable(name='slice_output_{}_0'.format(idx), dtype=np.int64, shape=[4])
        slice_output1 = gs.Variable(name='slice_output_{}_1'.format(idx), dtype=np.int64, shape=[4])
        inputs0 = [node.i(0).outputs[0], constant_n1, constant_nmax, constant_0, constant_n2]
        inputs1 = [node.i(0).outputs[0], constant_n2, constant_nmax, constant_0, constant_n2]

        slice_node0 = gs.Node(op='Slice', name='Slice_{}_0'.format(idx), inputs=inputs0, outputs=[slice_output0])
        slice_node1 = gs.Node(op='Slice', name='Slice_{}_1'.format(idx), inputs=inputs1, outputs=[slice_output1])

        concat_output = gs.Variable(name='concat_output_{}_0'.format(idx), dtype=np.int64, shape=[8])
        concat_node = gs.Node(
            op='Concat', name='Concat_{}_0'.format(idx), inputs=[slice_output0, slice_output1],
            outputs=[concat_output],
            attrs={'axis': 0})

        node.o().o().o().o().inputs = concat_node.outputs
        node.inputs.clear()
